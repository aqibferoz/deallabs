...
export class GuardiansComponent implements OnInit {
 ...
 guardians;
  ...
  ngOnInit() {    
    this.api.getGuardians().map(actions => {
  ...
  addChild(data){
    ...
    this.api.addChildren(data).then(res=>{
      this.selectedGuardian.totalChildren++;
      this.api.updateGuardian(this.selectedGuardian.id, { totalChildren:this.selectedGuardian.totalChildren}).then(resp=>{
